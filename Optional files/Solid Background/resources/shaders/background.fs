...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    number speed = 0.00000001 * (spin_time*SPIN_EASE*0.2) + 2.2;
    number new_pixel_angle = 0.0000001 * ((atan(uv.y, uv.x)) + speed - SPIN_EASE*20.*(1.*spin_amount*uv_len + (1. - 1.*spin_amount)));
   ...
   uv *= 30.;
    ...
    vec4 ret_col = 0.0000000001*((0.3/contrast)*colour_1 + (1. - 0.3/contrast)*((colour_1+0.000000001*c1p) + colour_2*c2p*0.000000001 + vec4(c3p*colour_3.rgb, c3p*colour_1.a))) + colour_1;
